...
| 模块 | 触发场景 | 入口 |
|------|---------|------|
| [`coze-agent`](./coze-agent/MODULE.md) | **【前置必读】** 任何操作前的授权检查、OAuth 授权流程控制、**防止未授权直接启动后台任务导致全链路失败** | `coze auth *` |
...
> **⚠️ 关键提醒：执行任何需要认证的 Coze CLI 操作前，必须先阅读并执行 [`coze-agent`](./coze-agent/MODULE.md) 完成授权检查！未完成授权就启动后台任务是导致全链路失败的常见原因，请务必遵守"前台授权 → 返回链接给用户 → 等待确认 → 验证状态 → 再新建后台任务"的强制流程。**
