...
**重要：开始之前请阅读完整文档。** 关键格式规则和常见问题在全文中都有涉及 - 跳过章节可能导致文件损坏或渲染问题。
...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      new Paragraph({ heading: HeadingLevel.HEADING_1, children: [new TextRun("一级标题")] }), // 使用覆盖的 Heading1 样式
...
- **设置默认字体**使用 `styles.default.document.run.font` - Arial 具有通用支持
- **建立视觉层次**使用不同的字体大小（标题 > 子标题 > 正文）
- **添加适当间距**使用 `before` 和 `after` 段落间距
- **谨慎使用颜色**：标题和子标题（一级标题、二级标题等）默认使用黑色 (000000) 和灰色调
- **设置一致的页边距**（1440 = 1 英寸是标准值）


## 列表（始终使用正确的列表 - 切勿使用 Unicode 符号）
```javascript
// 项目符号列表 - 始终使用 numbering 配置，不要使用 unicode 符号
// 关键：使用 LevelFormat.BULLET 常量，不是字符串 "bullet"
const doc = new Document({
  numbering: {
    config: [
  ...
  sections: [{
    children: [
      ...
      new Paragraph({ numbering: { reference: "bullet-list", level: 0 },
        children: [new TextRun("第一个项目要点")] }),
      new Paragraph({ numbering: { reference: "bullet-list", level: 0 },
        children: [new TextRun("第二个项目要点")] }),
      // 编号列表项
      ...
      new Paragraph({ numbering: { reference: "second-numbered-list", level: 0 },
        children: [new TextRun("重新从 1 开始（因为使用了不同的 reference）")] })
    ]
  }]
});
...
// 为每个单独的编号章节使用唯一的 reference 名称！

// ⚠️ 关键：切勿使用 unicode 项目符号 - 它们创建的假列表无法正常工作
// new TextRun("• 项目")           // 错误
...
new Table({
  columnWidths: [4680, 4680], // ⚠️ 关键：在表格级别设置列宽 - 值为 DXA（点的二十分之一）
      ...
      children: [
        new TableCell({
          ...
          children: [new Paragraph({ 
            alignment: AlignmentType.CENTER,
            children: [new TextRun({ text: "表头 1", bold: true, size: 22 })]
          })]
        }),
        new TableCell({
          borders: cellBorders,
          ...
          children: [new Paragraph({ 
            alignment: AlignmentType.CENTER,
            children: [new TextRun({ text: "项目要点", bold: true, size: 22 })]
          })]
        })
      ]
    }),
...
- 值为 DXA（点的二十分之一）：1440 = 1 英寸，Letter 纸张可用宽度 = 9360 DXA（1 英寸页边距）
- 将边框应用于单独的 `TableCell` 元素，而不是 `Table` 本身

**预计算的列宽（Letter 纸张 1 英寸页边距 = 总共 9360 DXA）：**
...
```javascript
// 带尺寸和定位的基本图片
// 关键：始终指定 'type' 参数 - 这是 ImageRun 必需的
new Paragraph({
  alignment: AlignmentType.CENTER,
  children: [new ImageRun({
    ...
    altText: { title: "Logo", description: "公司标志", name: "名称" } // 重要：三个字段都是必需的
  })]
})
```
...
// ⚠️ 关键：切勿单独使用 PageBreak - 这会创建 Word 无法打开的无效 XML
// ❌ 错误：new PageBreak() 
...
## 关键问题与常见错误
- **关键：PageBreak 必须始终在 Paragraph 内** - 单独的 PageBreak 会创建 Word 无法打开的无效 XML
- **表格单元格着色始终使用 ShadingType.CLEAR** - 切勿使用 ShadingType.SOLID（会导致黑色背景）
- 测量单位为 DXA（1440 = 1 英寸） | 每个表格单元格至少需要 1 个 Paragraph | 目录只需要 HeadingLevel 样式
- **始终使用自定义样式**配合 Arial 字体以获得专业外观和正确的视觉层次
- **始终设置默认字体**使用 `styles.default.document.run.font` - 推荐 Arial
- **表格始终使用 columnWidths 数组** + 单个单元格宽度以确保兼容性
- **切勿使用 unicode 符号作为项目符号** - 始终使用带有 `LevelFormat.BULLET` 常量的正确 numbering 配置（不是字符串 "bullet"）
- **切勿在任何地方使用 \n 换行** - 始终为每行使用单独的 Paragraph 元素
- **始终在 Paragraph children 中使用 TextRun 对象** - 切勿直接在 Paragraph 上使用 text 属性
- **图片关键点**：ImageRun 必须有 `type` 参数 - 始终指定 "png", "jpg", "jpeg", "gif", "bmp" 或 "svg"
- **项目符号关键点**：必须使用 `LevelFormat.BULLET` 常量，不是字符串 "bullet"，并包含 `text: "•"` 作为项目符号字符
- **编号关键点**：每个 numbering reference 创建一个独立列表。相同 reference = 继续编号（1,2,3 然后 4,5,6）。不同 reference = 从 1 重新开始（1,2,3 然后 1,2,3）。为每个单独的编号章节使用唯一的 reference 名称！
- **目录关键点**：使用 TableOfContents 时，标题必须只使用 HeadingLevel - 不要给标题段落添加自定义样式，否则目录会损坏
- **表格**：设置 `columnWidths` 数组 + 单个单元格宽度，将边框应用于单元格而非表格
- **在表格级别设置表格边距**以获得一致的单元格内边距（避免在每个单元格上重复设置）
